...
namespace ZoomLaCMS.Manage.Content
{
    public partial class AddJsContent : CustomerPageAction
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (jinfo.id > 0)
                {
                    ...
                    this.ContentType_Rad.SelectedIndex = jinfo.ContentType;
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            Mjs.JsFileName = this.JsFileName.Text;
            Mjs.ContentType = this.ContentType_Rad.SelectedIndex;
            Mjs.JsType = 0;
            ...
            switch (txt_ShowType)
            {
                ...
                case "4"://各项独立式--框架显示
                    ...
                    string folderPath = FileSystemObject.CreateFileFolder(Server.MapPath("/JS/Ads/" + ContentType_Rad.SelectedValue + "/" + FolderName)); //创建文件夹
                    DataTable dt = FileSystemObject.GetFileList(folderPath);  //获取该文件夹下所有文件 
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        DataRow dr = dt.Rows[i];
                        //删除该文件夹下所有文件
                        FileSystemObject.Delete((Server.MapPath("/JS/Ads/" + ContentType_Rad.SelectedValue + "/" + FolderName + "/" + dr[0].ToString())), FsoMethod.File);
                    }
                    ...
                    if (fileNames != null && fileNames.Count > 0)
                    {
                        ...
                        if (ContentType_Rad.SelectedValue.Equals("JS")) //写入
                        {
                        ...
                        if (ContentType_Rad.SelectedValue.Equals("HTML"))
                        {

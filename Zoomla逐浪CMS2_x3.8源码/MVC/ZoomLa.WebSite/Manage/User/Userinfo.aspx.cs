...
namespace ZoomLaCMS.Manage.User
{
    public partial class Userinfo : CustomerPageAction
    {
        ...
        private DataTable UPProDT { get { return (ViewState["UPProDT"] as DataTable); } set { ViewState["UPProDT"] = value; } }
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (mu.IsNull) { function.WriteErrMsg("用户[" + UserID + "]不存在"); }
                ...
                if (DataConvert.CStr(Request["show"]) == "diag") { Call.HideBread(Master); }
                else { Call.SetBreadCrumb(Master, "<li><a href='" + CustomerPageAction.customPath2 + "Main.aspx'>工作台</a></li><li><a href='AdminManage.aspx'>用户管理</a></li><li><a href='UserManage.aspx'>会员管理</a></li><li class='active'><a href='" + Request.RawUrl + "'>会员信息</a> 当前用户:" + mu.UserName + "</li>"); }
            }
        }
        //绑定用户角色
        ...
        //-------作者
        B_Author authorBll = new B_Author();
        M_Author authorMod = new M_Author();
        ...
        protected void AddUPro_Btn_Click(object sender, EventArgs e)
        {
            ...
            M_User_BindPro ubpMod = ubpBll.SelModelByUid(UserID);
            if (ubpMod == null) { ubpMod = new M_User_BindPro(); }
            ubpMod.ProIDS = StrHelper.PureIDSForDB(ids);
            if (ubpMod.ID > 0) { ubpBll.UpdateByID(ubpMod); }
            else
            {
                M_UserInfo mu = buser.GetUserByUserID(UserID);
        ...
        protected void UPRONode_RPT_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                DataRowView drv = e.Item.DataItem as DataRowView;
                Repeater RPT = e.Item.FindControl("UPro_RPT") as Repeater;
                UPProDT.DefaultView.RowFilter = "NodeID=" + drv["NodeID"];
                DataTable dt = UPProDT.DefaultView.ToTable();
                RPT.DataSource = dt;
                RPT.DataBind();
            }
        }
        protected void UserGroup_DP_SelectedIndexChanged(object sender, EventArgs e)
        {
            int gid = DataConvert.CLng(UserGroup_DP.SelectedValue);
            buser.UpdateGroupId(UserID.ToString(), gid);
            Response.Redirect("UserInfo.aspx?ID=" + UserID + "&tabs=Tabs4");
        }
    }
}
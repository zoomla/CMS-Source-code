...
var scence = {
    ...
    pageid: "",
    tempcheck: true,//用于选择元素时的临时判断，如打开元素列表不显示遮罩
};
//删除当前页
scence.del = function () {
    var ref = this;
...
tools.show = function () {
    ...
    tools.menu.show();
    tools.menu.css("top", dom.offset().top - 50).css("left", dom.offset().left + 70);
}
//------------------------------上传
...
mbh5.preview = function () {
    mask.wait();
    PreSave(function () {
        if (page.sid && page.sid != 0) { mbh5.pop.show("/h5/" + page.sid); }
        else { mbh5.pop.show("/design/h5/preview.aspx?id=" + page.guid); }

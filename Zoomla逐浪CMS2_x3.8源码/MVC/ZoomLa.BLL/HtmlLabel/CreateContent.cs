            ...
            if (nll.SelByIDS(this.Nodeist).Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ntable.Rows.Count; i++)
                {
                    ...
                    for (int b = 0; b < GetContent.Rows.Count; b++)
                    {
                        ...
                        {
                            string ContentPath = MakeHtmlFile(nll.SelReturnModel(DataConverter.CLng(NodeID))) + GetPath(NodeID, GeneralID);
                            ContentPath = ContentPath.Replace(@"\//", @"\");
                            ContentPath = ContentPath.Replace("/", @"\");
                    ...
                    if (Pagelist.IndexOf(urlstr) > -1)
                    {
                        ...
                        if (urlstr.IndexOf("=") > -1)
                        {
                            ...
                            if (strlink.Length > 0)
                            {
                                ...
                                string pageurlstr = pageno + "_" + pageitemid + GetNodeContentEx(nll.SelReturnModel(DataConverter.CLng(NodeID)));
            ...
            if (results.IndexOf("NodeID=") > -1)
            {
                ...
                if (listarr.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        nodeurl = nll.SelReturnModel(nodeid).NodeListUrl;
                        Defaultxt = Defaultxt.Replace(results, nodeurl);
                    }
                }
            }

namespace ZoomLa.BLL
{
    ...
    public class B_User
    {
        ...
        public DataTable SelPage(int psize, int cpage, out int itemCount, string action, string value, string order, string charstr, int status)
        {
            ...
            switch (action)//筛选条件
            {
                ...
                case "honeyname":
                    ...
                    break;
                case "truename":
                    where="A.[PERMISSIONS] LIKE @uname";
                    break;
                case "name":
        ...
        public static bool CheckUserStatus(M_UserInfo mu, ref string err)
        {
            bool flag = true;
        ...
        public static bool CheckIsLogged(string returnUrl = "")
        {
        ...
        public string SelParentTree(int uid)
        {
        ...
        /// <returns></returns>
        public string SelParentTree(string tbname, string pk, string pfield, int startID)
        {
            string ids = "";
            try
            {
        ...
        /// 获取子级链,包含起始ID
        /// </summary>
        public string SelChildTree(string tbname, string pk, string pfield, int startID)
        {
            string ids = "";
        ...
        public PageSetting QueryUser(int cpage, int psize, int sex = 0, string username = "", int userid = 0)
        {
            string where = "1=1";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (sex > 0) { where += " AND B.UserSex=" + sex; }
            if (!string.IsNullOrEmpty(username)) { where += " AND A.UserName LIKE @uname"; sp.Add(new SqlParameter("uname", "%" + username + "%")); }
            if (userid > 0) { where += " AND A.UserID=" + userid; }
            PageSetting setting = PageSetting.Double(cpage, psize, strTableName, strTableName2, "A.UserID", "A.UserID=B.UserID", where, "A.RegTime DESC", sp);
            DBCenter.SelPage(setting);
            return setting;
        }

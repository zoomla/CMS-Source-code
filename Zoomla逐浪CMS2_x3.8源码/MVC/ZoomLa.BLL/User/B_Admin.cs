namespace ZoomLa.BLL
{
    ...
    public class B_Admin : B_Base<M_AdminInfo>
    {
        ...
        public static bool CheckIsLogged(string url = "")
        {
            string loginUrl = "~/" + SiteConfig.SiteOption.ManageDir + "/Login.aspx?r=" + url;
            if (HttpContext.Current.Session["M_LoginName"] != null && HttpContext.Current.Session["M_Password"] != null) { return true; }
            if (HttpContext.Current.Request.Cookies["ManageState"] == null)
            {
                HttpContext.Current.Response.Redirect(loginUrl); return false;
            }
            else
            {
                ...
                if (!string.IsNullOrEmpty(loginName)) { loginName = StringHelper.Base64StringDecode(loginName); }
                M_AdminInfo adminMod = new B_Admin().GetLoginAdmin(loginName, password);
                if (adminMod == null) { HttpContext.Current.Response.Redirect(loginUrl); return false; }
                else if (adminMod.IsLock) { throw new Exception("该管理员账户已被锁定"); }

...
public partial class manage_Shop_OtherOrder_TravelOrder_Manager : CustomerPageAction
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Call.SetBreadCrumb(Master, "<li><a href='" + CustomerPageAction.customPath2 + "Main.aspx'>工作台</a></li><li><a href='../../Shop/ProductManage.aspx'>商城管理</a></li><li><a href='../../Shop/OrderList.aspx'>订单管理</a></li><li class='active'>旅游订单管理</li>");
    }
    ...
    private void Bind()
    {
        ...
        switch (menu == "souch" && key != "")
        {
            case true:
                ...
                Cll = bll.Getsouchinfo(soutype, key, 3);
                break;
            case false:
                if (getquicksou > 0)
                {
                    Cll = bll.GetOrderListtype(getquicksou, 3);
                }
                else
                {
                    Cll = bll.GetOrderListByOrderType(3);
                }
                break;
            default:
                Cll = bll.GetOrderListByOrderType(3);
        ...
        try
        {
            DataTable allorders = bll.GetOrderListByOrderType(3);
            double tempss = 0;
    ...
    public string getorderno(string id)
    {
        ...
        if (aside == 1)
        {
            sss = "<a href=\"Travel_Orderinfo.aspx?id=" + cdd.id + "\"><font color=#cccccc>" + cdd.OrderNo + "</font></a>";
        }
        else
        {
            sss = "<a href=\"Travel_Orderinfo.aspx?id=" + cdd.id + "\">" + cdd.OrderNo + "</a>";
        }
        return sss;
    }
    ...
    protected void souchok_Click(object sender, EventArgs e)
    {
        ...
        if (key != "")
        {
            Response.Redirect("TravelOrder_Manager.aspx?menu=souch&souchtable=" + soutype + "&souchkey=" + key + "");
        }
    }

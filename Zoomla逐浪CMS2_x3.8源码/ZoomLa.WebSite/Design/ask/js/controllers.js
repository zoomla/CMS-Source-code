...
.controller("AskEditCtrl", function ($scope, $state, $stateParams,Loading, Ask) {
    ...
    //------different
    Loading.show();
    Ask.get($stateParams.id, function (result) { $scope.model = result; $scope.model.EndDate=new Date($scope.model.EndDate); $scope.$digest();Loading.hide(); });
})
.controller("AskQListCtrl", function ($scope, $state, $stateParams, $ionicPopup, Ask,User, Loading, Question, DB) {
    ...
    Loading.show();
    Ask.get($stateParams.askid, function (result) { $scope.askMod = result; console.log(result)});
...
.controller("AskViewCtrl", function ($scope, $state, $stateParams, $ionicPopup, $ionicScrollDelegate, Loading, Ask, Question, Answer, DB) {
	...
	$scope.ShowShare = function(){ $scope.showShare = true; };

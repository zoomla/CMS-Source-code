namespace ZoomLa.BLL
{
    ...
    public class B_Cart
    {
        ...
        public void UpdateProNum(string cartid, int id, int pronum)
        {
            SqlParameter[] sp = new SqlParameter[] { new SqlParameter("cartid", cartid) };
            string sql = "Update " + TbName + " Set ProNum=" + pronum + " Where Cartid=@cartid And ID=" + id;
        ...
        public string GetCartAll(string charid)
        {
            ...
            SqlDataReader dr = SqlHelper.ExecuteReader(sql,sp);
            if (dr.Read())
            {
        ...
        public bool DelByID(string cartid, int id)
        {
            return DelByIDS(cartid, id.ToString());
        }
        public bool DelByIDS(string cartid, string ids)
        {
            SafeSC.CheckIDSEx(ids);
            SqlParameter[] sp = new SqlParameter[] { new SqlParameter("cartid", cartid) };
            string sql = "Delete From ZL_Cart Where ID IN(" + ids + ") And CartID=@cartid";

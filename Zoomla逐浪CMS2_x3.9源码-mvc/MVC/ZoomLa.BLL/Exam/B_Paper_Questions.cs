namespace ZoomLa.BLL
{
    ...
    public class B_Paper_Questions
    {
		...
		/// <returns></returns>
		public int GetInsert(M_Paper_Questions paperQuestions)
        {
            string sqlStr = "INSERT INTO [dbo].[ZL_Paper_Questions] ([QuestionTitle],[PaperID],[OrderBy],[QuestionType],[Subtitle],[Course],[QuesNum],[Remark],[QuestIDs],[CreateTime],[AddUser]) VALUES (@QuestionTitle,@PaperID,@OrderBy,@QuestionType,@Subtitle,@Course,@QuesNum,@Remark,@QuestIDs,@CreateTime,@AddUser);select @@IDENTITY";
            SqlParameter[] cmdParams = initmod.GetParameters(paperQuestions);
		...
		/// <param name="Paper_Questions"></param>
		/// <returns></returns>
		public bool GetUpdate(M_Paper_Questions paperQuestions)
        {
            string sqlStr = "UPDATE [dbo].[ZL_Paper_Questions] SET [QuestionTitle] = @QuestionTitle,[PaperID] = @PaperID,[OrderBy] = @OrderBy,[QuestionType] = @QuestionType,[Subtitle] = @Subtitle,[Course] = @Course,[QuesNum] = @QuesNum,[Remark] = @Remark,[QuestIDs] = @QuestIDs,[CreateTime] = @CreateTime,[AddUser] = @AddUser WHERE [ID] = @ID";
            SqlParameter[] cmdParams = initmod.GetParameters(paperQuestions);

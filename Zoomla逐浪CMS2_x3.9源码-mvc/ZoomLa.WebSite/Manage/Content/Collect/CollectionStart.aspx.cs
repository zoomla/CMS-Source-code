    ...
    public partial class CollectionStart : CustomerPageAction
    {
        ...
        protected void btnCollAll_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Request.Form["idchk"]))
            {
                ...
                {
                    worker.CreateCollectionProc();
                }
                else
                {
                    worker = (B_Coll_Worker)Application["MailSends"];
                }
                B_CollectionItem CollectionItem = new B_CollectionItem();
                DataTable citem = CollectionItem.Select_All();

                if (citem.Rows.Count > 0)
                {
                    DataTable citemlist = CollectionItem.SelBySwitch(1);
                    if (citemlist.Rows.Count > 0)
                    {
                        B_Coll_Worker.StopColl = false;
                    }
                }

namespace ZoomLa.BLL
{
    ...
    public class B_OrderList
    {
        ...
        public DataTable Search(string orderType, string logistics, string orderStatus, string payStatus, string stime, string etime, int storeid, string uname, string uids, string quickSql, string skeySql, List<SqlParameter> sp)
        {
            ...
            if (!string.IsNullOrEmpty(stime)) { where += " AND AddTime >=@stime"; sp.Add(new SqlParameter("stime", stime)); }
            if (!string.IsNullOrEmpty(etime)) { where += " AND AddTime <=@etime"; sp.Add(new SqlParameter("etime", etime)); }
        ...
        public DataTable SearchByQuickAndSkey(string orderType, string orderStatus, string payStatus, int quick, int type, string skey, int storeid = -100, string uids = "")
        {
            string logistics = "", stime = "", etime = "", uname = "";
            ...
            if (!string.IsNullOrEmpty(skey))
            {
                ...
                switch (type)
                {
                    ...
                    case 13:
                        ...
                        break;
                    case 14:
                        skeySql += " AND parentID=@keyinfo ";
                        break;
                    case 15:
                        skeySql += " AND Promoter=" + DataConverter.CLng(skey);
                        break;
                    default:
                        skeySql += " AND OrderNo like @keyinfo ";
                        break;
        ...
        public bool DeleteByID(int OrderListId)
        {
            return Sql.Del(TbName, OrderListId);
        }
        ...
        public bool Delorderlist(string ids)
        {
            DelByIDS(ids); return true;
        }
        //----------------------------Tools

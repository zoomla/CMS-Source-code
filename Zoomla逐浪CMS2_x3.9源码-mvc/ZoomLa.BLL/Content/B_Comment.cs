namespace ZoomLa.BLL
{
    ...
    public class B_Comment
    {
        ...
        //public DataTable SelByGid(int gid) 
        //{
        //    string sql = "Select * From " + TbName + " Where GeneralID=" + gid + " And Pid=0 And Audited=1";
        ...
        public bool DelByCIDs(string commentIds)
        {
            string[] commentIdsArr = commentIds.Split(',');
            string sql = "";
            SqlParameter[] sp = new SqlParameter[commentIdsArr.Length];
            ...
            sql = sql.TrimEnd(',');
            int result = SqlHelper.ExecuteNonQuery(CommandType.Text, "delete from dbo.ZL_Comment where commentID in (" + sql + ")", sp);
            if (result > 0)
            {
                return true;
            }
        ...
        {
            DBCenter.DelByWhere(TbName, "CommentID=" + id + " AND UserID=" + uid);
        }
    }
}

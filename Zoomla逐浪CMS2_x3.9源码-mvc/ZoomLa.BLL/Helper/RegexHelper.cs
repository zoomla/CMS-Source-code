...
namespace ZoomLa.BLL
{
    public class RegexHelper
    {
        ...
        #endregion
        #region 校验区
        public bool IsEmail(string s) { if (string.IsNullOrEmpty(s))return false; return Regex.IsMatch(s, S_Email); }
        //手机号码
        public bool IsMobilPhone(string s) { if (string.IsNullOrEmpty(s))return false; return Regex.IsMatch(s, S_Mobile); }
        //身份证格式
        public bool IsIDCard(string s) { if (string.IsNullOrEmpty(s))return false; return Regex.IsMatch(s, @"\d{15}|\d{18}"); }
        //邮政编码
        public bool IsPostCode(string s) { if (string.IsNullOrEmpty(s))return false; return Regex.IsMatch(s, @"[1-9]\d{5}(?!\d)"); }
        //Http或Https地址,示例:http://www.z01.com,可以/结尾
        public bool IsHttpUrl(string s) { if (string.IsNullOrEmpty(s))return false; return Regex.IsMatch(s, @"^(http|https)://([\w-]+\.)+[\w-]+(/[\w-./?%&=]*)?$", RegexOptions.IgnoreCase); }
        #endregion
        #region 用于服务端控件的字符串

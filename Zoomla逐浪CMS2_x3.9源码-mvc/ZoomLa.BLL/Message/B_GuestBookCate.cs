...
namespace ZoomLa.BLL
{
    public class B_GuestBookCate : ZL_Bll_InterFace<M_GuestBookCate>
    {
        ...
        public DataTable SelPage(out int count, int status, int CPage, int CateID, int pid, string Skey = "")
        {
            SqlParameter[] sp = new SqlParameter[] { new SqlParameter("skey", "%" + Skey + "%") };
            string wherestr = "1=1";
            wherestr += CateID > 0 && status != -1 ? " AND A.CateID=" + CateID : "";
            if (status != 100) { wherestr += " And A.Status=" + status; }
            else { wherestr += " AND A.Status<>-1"; }
            if (!string.IsNullOrEmpty(Skey)) { wherestr += " AND A.Title Like @skey OR A.Gid=" + DataConvert.CLng(Skey); }
            ...
            return DBCenter.SelPage(10, CPage, out count, "A.GID", "A.*,B.UserName,B.HoneyName,B.salt AS UserFace", "("+mtable+")", "ZL_User", "A.UserID=B.UserID", wherestr, "GDate Desc", sp);
        }
    }
}

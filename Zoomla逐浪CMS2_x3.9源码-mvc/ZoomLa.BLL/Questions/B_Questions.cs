namespace ZoomLa.BLL
{
    ...
    public class B_Questions
    {
        ...
        {
            return Sql.UpdateByIDs(strTableName, PK, model.p_id.ToString(), BLLCommon.GetFieldAndPara(model), model.GetParameters(model));
        }
        ...
        public int GetInsert(M_Questions questions)
        {
            string sqlStr = "INSERT INTO [dbo].[ZL_Questions] ([p_title],[p_Difficulty],[p_Class],[p_Views],[p_Inputer],[p_Type],[p_Knowledge],[p_Answer],[p_Content],[p_CreateTime],[paper_Id],[p_Order]) VALUES (@p_title,@p_Difficulty,@p_Class,@p_Views,@p_Inputer,@p_Type,@p_Knowledge,@p_Answer,@p_Content,@p_CreateTime,@paper_Id,@p_Order);select @@IDENTITY";
            SqlParameter[] cmdParams = initMod.GetParameters(questions);

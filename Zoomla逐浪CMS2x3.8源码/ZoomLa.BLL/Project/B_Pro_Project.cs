...
namespace ZoomLa.BLL.Project
{
    public class B_Pro_Project : ZL_Bll_InterFace<M_Pro_Project>
    {
        ...
        public PageSetting SelPage(int cpage, int psize, string name = "", string manageer = "", string tecDirector = "", string sdate = "", string edate = "", int type = -100, int status = -100)
        {
            string where = "";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (!string.IsNullOrEmpty(name)) { where += " AND ProName LIKE @name"; sp.Add(new SqlParameter("name", "%" + name + "%")); }
            if (!string.IsNullOrEmpty(manageer)) { where += " AND ProManageer LIKE @manageer"; sp.Add(new SqlParameter("manageer", "%" + manageer + "%")); }
            if (!string.IsNullOrEmpty(tecDirector)) { where += " AND TecDirector LIKE @tec"; sp.Add(new SqlParameter("tec", tecDirector)); }
            ...
            if (type != -100) { where += " AND ZType=" + type; }

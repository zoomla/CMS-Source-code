namespace ZoomLa.BLL
{
    ...
    public class B_Exam_Sys_Questions
    {
        ...
        public DataTable U_SelByFilter(int classid, int qtype, string title, int uid = 0, int issmall = -1)
        {
            ...
            if (uid > 0)
            {
                sql += " AND(UserID=" + uid + " OR IsShare=1)";
            }
            if (issmall > -1)
            {

...
namespace ZoomLa.BLL
{
    ...
    {
        private string pubCodeLabel = "<input type='text' id='VCode' placeholder='验证码' maxlength='6' runat='server' class='codestyle' style='display:inline;' />"
                                    + "<img id='VCode_img' title='点击刷新验证码' class='codeimg'/><input type='hidden' id='VCode_hid' name='VCode_hid' />"
                                    + "<script> $('#VCode').ValidateCode();</script>";
                    ...
                    if (pubinfos.Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (PubContentid == 0)
                                {
                                    PubContentid = DataConvert.CLng(GetIDVal(CurrentReq.Request.RawUrl.ToLower()));
        ...
        public string GetRequest(string html)
        {
            ...
            string url = CurrentReq.Request.RawUrl.ToLower();

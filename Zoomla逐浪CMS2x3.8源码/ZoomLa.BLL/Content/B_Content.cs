...
namespace ZoomLa.BLL
{
    public class B_Content
    {
        ...
        //------------
        public PageSetting SelContent(int cpage, int psize, int nodeid, string status, string inputer, string skey)
        {
            string where = "";
        ...
        {
            List<SqlParameter> sp = new List<SqlParameter>() { new SqlParameter("tablename", "%" + TableName + "%") };
            string where = " TableName LIKE @tablename ";
            switch ((flag ?? "").ToLower())
            {
                case "audit":
                    where += " AND Status=99";
                    break;
                case "elite":
                    where += " AND EliteLevel>0";
                    break;
                default:
                    where += " AND Status !=" + (int)ZLEnum.ConStatus.Recycle;
                    break;
            }
            if (!string.IsNullOrEmpty(keyword))
            {
            ...
            {
                where += " AND Title Like @keyword ";
                sp.Add(new SqlParameter("keyword", "%" + keyword + "%"));
            }
            if (Nodeid > 0)
            {
            ...
            if (!string.IsNullOrEmpty(Inputer))
            {
                where += " AND Inputer Like @Inputer ";
                sp.Add(new SqlParameter("Inputer", Inputer));
            }
            return DBCenter.Sel(TbName, where, "OrderID Desc", sp);
        }
        /// <summary>
        /// 读取回收站内的内容列表
        /// </summary>

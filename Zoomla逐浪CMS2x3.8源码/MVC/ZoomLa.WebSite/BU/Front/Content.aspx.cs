...
namespace ZoomLaCMS
{
    public partial class Content : FrontPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string title = B_Route.GetParam("ID", this);
            if (ItemID > 0)
            {
                ItemInfo = conBll.GetCommonData(ItemID);
                nodeMod = nodeBll.SelReturnModel(ItemInfo.NodeID);
                if (nodeMod == null || nodeMod.IsNull) { ErrToClient("[产生错误的可能原因：(" + ItemInfo.NodeID + ")节点不存在!]"); }
                if (nodeMod.ItemOpenTypeTrue.Equals("cn")) { ErrToClient("[产生错误的可能原因：内容信息不存在或未开放!调用方法：/Item/标题]"); }
            }
            else if (!string.IsNullOrEmpty(title))
            {
                ...
                nodeMod = nodeBll.SelReturnModel(ItemInfo.NodeID);
                if (nodeMod == null || nodeMod.IsNull) { ErrToClient("[产生错误的可能原因：(" + ItemInfo.NodeID + ")节点不存在!]"); }
                if (!nodeMod.ItemOpenTypeTrue.Equals("cn")) { ErrToClient("[产生错误的可能原因：内容信息不存在或未开放!调用方法：/Item/内容GID]"); }
            }
            else { ErrToClient("[产生错误的可能原因：内容信息不存在或未开放!调用方法：/Item/内容GID]"); }
            ...
            if (ItemInfo.Status == (int)ZLEnum.ConStatus.Recycle) { ErrToClient("[当前信息已删除，您无法浏览!]"); }
            else if (ItemInfo.Status == (int)ZLEnum.ConStatus.UnAudit) { ErrToClient("[当前信息待审核状态，您无法浏览!]"); }
            else if (ItemInfo.Status != (int)ZLEnum.ConStatus.Audited) { ErrToClient("[当前信息未通过审核，您无法浏览!]"); }

            if (nodeMod.PurviewType)
            {
                if (!buser.CheckLogin())
                {
                    function.WriteErrMsg("该信息所属栏目需登录验证，请先<a href='/User/login' target='_top'>登录</a>再进行此操作！", "/User/login");
                }
            }
            //-------------------------End;
            ...
            if (string.IsNullOrEmpty(TemplateDir))
            {
                function.WriteErrMsg("该内容所属模型未指定模板"); return;
            }

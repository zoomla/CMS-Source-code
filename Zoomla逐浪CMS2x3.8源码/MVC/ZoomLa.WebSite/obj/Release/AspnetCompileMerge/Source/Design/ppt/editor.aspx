<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="editor.aspx.cs" Inherits="ZoomLaCMS.Design.ppt.editor" MasterPageFile="~/Common/Master/Empty2.master" EnableViewState="false"%>
...
<div id="tools">
    ...
    <div id="toolsmenu_div" class="btn-group" style="position: absolute;display:none;z-index:1000;">
...
<script>
    ...
    $(function () {
        ...
        $("#editorBody").on('click', '.comp_wrap', function (event, istrigger) {
            ...
            tools.menu.show();
            tools.menu.css("top", $obj.offset().top - 40).css("left", $obj.offset().left);
            console.log(tools.menu.length);
    ...
    myani.init = function () {
        myani.swiper = new Swiper('.swiper-container', {
            ...
            onInit: function (swiper) {
                ...
                top.scence.ppt.initbg();
            },
            onSlideChangeEnd: function (swiper) {
                //必须加这句,否则修改过的style无效
                swiperAnimateCache(swiper); //隐藏动画元素 
                swiperAnimate(swiper);  //每个slide切换结束时也运行当前slide动画
                top.scence.ppt.loadbg(swiper.wrapper[0].childNodes[swiper.activeIndex], swiper.activeIndex);
            }
        });
    }
    //------------------------
    ...
    tools.bind = function () {
        ...
        top.editor.ShowDiag("/design/diag/BindEvent.aspx?id=" + tools.comp.id);
    }
    tools.del = function () {
    ...
    tools.animate = function () {
        top.editor.ShowDiag("/design/diag/common/animate.aspx?id=" + tools.comp.id);
    }
    tools.edit = function () {
        var url = "";
        var comp = tools.comp;
        ...
        top.editor.ShowDiag(url, comp.diagParam);
    }
    tools.common = function () { top.editor.ShowDiag("/design/diag/common/edit.aspx?id=" + tools.comp.id); }

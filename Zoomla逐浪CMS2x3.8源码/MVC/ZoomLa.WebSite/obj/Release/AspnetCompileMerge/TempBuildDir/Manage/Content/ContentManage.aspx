    ...
    pop.bindEvent = function ($items) {
        $items.popover({
            animation: true, placement: 'left', title: "扫描二维码浏览", content: function () {
                var url = '<%=ZoomLa.Components.SiteConfig.SiteInfo.SiteUrl%>/Item/' + pop.gid + '.aspx';
                return '<div><img src="/common/common.ashx?url=' + url + '" style="width:150px;height:150px;"></div>';
            }, html: true, trigger: 'manual',
        }).hover(function () {
            var $uinfo = $(this);
            pop.gid = $uinfo.data("gid");
            $uinfo.popover('show');
        }, function () {
            $(this).popover('hide');
        })

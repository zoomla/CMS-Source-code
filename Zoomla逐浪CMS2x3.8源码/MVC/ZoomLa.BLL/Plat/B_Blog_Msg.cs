...
namespace ZoomLa.BLL.Plat
{
    public class B_Blog_Msg : ZL_Bll_InterFace<M_Blog_Msg>
    {
        ...
        public void AddRead(string ids)
        {
            ...
            {
                paraIds += GetReadIDS(DataConvert.CLng(id));
            }
            paraIds = paraIds.Trim(',');
            if (!string.IsNullOrEmpty(paraIds))
            {
                DBCenter.UpdateSQL(TbName, "ReadNumber+=1", "ID IN (" + paraIds.Trim(',') + ")");
            }
        }
        private string GetReadIDS(int id)
        {
            string fids = "";
            ...
            {
                fids += msgMod.ID + ",";
                if (msgMod.ForWardID > 0 && msgMod.ForWardID == msgMod.ID) { fids += GetReadIDS(msgMod.ForWardID); }
            }
            return fids;
        }
        /// <summary>

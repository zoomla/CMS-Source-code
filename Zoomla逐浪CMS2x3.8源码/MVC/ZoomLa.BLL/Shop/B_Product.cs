...
namespace ZoomLa.BLL
{
    ...
    public class B_Product
    {
        ...
        /// 筛选商品信息
        /// </summary>
        /// <param name="storeid">-100全部,0仅商城,>0指定商铺</param>
        /// <param name="skey">商品名关键词</param>
        /// <param name="filter">特殊筛选条件</param>
        public PageSetting SelPage(int cpage, int psize, int storeid, int nodeid, string skey, int filter, bool hasRecycle = false)
        {
            List<SqlParameter> sp = new List<SqlParameter>();
            string where = "1=1 ";
            if (storeid != -100) { where += " AND UserShopID=" + storeid; }
            if (nodeid > 0) { where += " AND NodeID=" + nodeid; }
            if (!string.IsNullOrEmpty(skey)) { where += " AND ProName LIKE @skey"; sp.Add(new SqlParameter("skey", "%" + skey + "%")); }
            if (!hasRecycle) { where += " AND Recycler=0"; }
            where += GetWhereByType(filter);
            PageSetting setting = PageSetting.Single(cpage, psize, TbName, PK, where, "", sp);
            DBCenter.SelPage(setting);
            return setting;
        }
        public PageSetting SelPageByKayword(int cpage, int psize, string kayword = "", string key = "")
        {
            string where = "1=1";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (!string.IsNullOrEmpty(kayword)) { where += " AND Kayword LIKE @kayword"; sp.Add(new SqlParameter("kayword", "%" + kayword + "%")); }
            if (!string.IsNullOrEmpty(key)) { where += " AND ProName LIKE @key"; sp.Add(new SqlParameter("key", "%" + key + "%")); }
            PageSetting setting = PageSetting.Single(cpage, psize, TbName, PK, where, "", sp);
            DBCenter.SelPage(setting);
            return setting;
        }
        ...
        {
            string wherestr = GetWhereByType(type);
        ...
        public DataTable ProductSearch(int nodeid, int Type, string KeyWord, int storeid = 0)
        {
            ...
            if (nodeid > 0) { sqlstr += " AND NodeID =" + nodeid; }
            if (storeid == 0) { sqlstr += " AND UserShopID=0 OR UserShopID IS NULL"; }
            else if (storeid > 0)
            {
        ...
        public bool setproduct(int type, string list)
        {
            ...
            {
                case 1:
                    sqlStr = "update ZL_Commodities set Sales=1 where (ID in (" + list + "))";
                    break;
                case 2:
                    sqlStr = "update ZL_Commodities set ishot=1 where (ID in (" + list + "))";
                    break;
                case 3:
                    sqlStr = "update ZL_Commodities set isbest=1 where (ID in (" + list + "))";
                    break;
                case 4:
                    sqlStr = "update ZL_Commodities set isnew=1 where (ID in (" + list + "))";
                    break;
                case 5:
                    try
                    {
                        string tempsql = "select Tablename,ItemID from ZL_Commodities where (ID in (" + list + "))";
                    ...
                    sqlStr = "delete from ZL_Commodities where (ID in (" + list + "))";
                    break;
                case 6:
                    sqlStr = "update ZL_Commodities set Sales=0 where (ID in (" + list + "))";
                    break;
                case 7:
                    sqlStr = "update ZL_Commodities set ishot=0 where (ID in (" + list + "))";
                    break;
                case 8:
                    sqlStr = "update ZL_Commodities set isbest=0 where (ID in (" + list + "))";
                    break;
                case 9:
                    sqlStr = "update ZL_Commodities set isnew=0 where (ID in (" + list + "))";
                    break;
                case 10://批量审核
                    sqlStr = "update ZL_Commodities set istrue=1 where (ID in (" + list + "))";
                    break;
                case 11://取消审核
                    sqlStr = "update ZL_Commodities set istrue=0 where (ID in (" + list + "))";
                    break;
                case 12://还原
                    sqlStr = "update ZL_Commodities set Recycler=0 where (ID in (" + list + "))";
                    break;
                case 13://进回收站
                    sqlStr = "update ZL_Commodities set Recycler=1 where (ID in (" + list + "))";
                    break;
                case 14://全部还原
                    sqlStr = "update ZL_Commodities set Recycler=0 where Recycler=1";
                    break;
                case 15://清空回收站
                    DataTable dt = SqlHelper.ExecuteTable(CommandType.Text, "SELECT * FROM " + TbName + " WHERE Recycler=1");
                    string ids = "", items = "";
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        items += dt.Rows[i]["ItemID"] + ",";
                    ...
                    return true;
                default:
                    sqlStr = "update ZL_Commodities set isnew=isnew where (ID in (" + list + "))";
                    break;
            }
        ...
        public bool ImportProducts(DataTable dt, int ModelID, int NodeID)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    throw new Exception("第" + (i + 1) + "行，字段[服务期限]格式不正确");
                }
                ...
                {
                    throw new Exception("第" + (i + 1) + "行，字段[前台库存计算方式]格式不正确");
                }
                try
                {
                    m_Product.Rateset = 1;
                ...
                {
                    throw new Exception("第" + (i + 1) + "行，字段[预订时间]格式不正确");
                }
                try
                {
                ...
                {
                    throw new Exception("第" + (i + 1) + "行，字段[礼品价格]格式不正确");
                }
                ...
                {
                    throw new Exception("第" + (i + 1) + "行，字段[促销方案]格式不正确");
                }
                try
                {
                ...
                {
                    throw new Exception("第" + (i + 1) + "行，字段[秒杀时间段]格式不正确");
                }
                m_Product.Preset = "";
        ...
        public void GetNo(int NodeID)
        {
            ...
            M_Node nodeinfo = b_Node.SelReturnModel(NodeID);
        ...
        public static string GetProCode()
        {
            ...
            int count = Convert.ToInt32(SqlHelper.ExecuteScalar(CommandType.Text, "SELECT Count(ID) From ZL_Commodities WHERE AddTime BETWEEN '" + stime + "' AND '" + etime + "'"));
            count++;
        ...
        /// <summary>
        /// 用于购物车车与订单页,同时显示商品的属性
        /// </summary>
        //public string GetProName(M_Product proMod, string attrs)
        //{
        //    if (string.IsNullOrEmpty(attrs)) { return proMod.Proname; }
        //    string result = "";
        ...
        //        result += value + " ";
        //    }
        //    if (string.IsNullOrEmpty(result)) { return proMod.Proname; }
        //    else { return proMod.Proname += "(" + result.Trim() + ")"; }
        //}
        //-------------------------------会员价逻辑
        /// <summary>

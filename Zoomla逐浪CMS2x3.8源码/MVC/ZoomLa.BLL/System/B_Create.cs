    ...
    public class B_Create
    {
        ...
        public void CreateList(int NodeID)
        {
            try
            {
                ...
                string TemplateDir = SiteConfig.SiteOption.TemplateDir;
                string GeneratedDirectory = SiteConfig.SiteOption.GeneratedDirectory;
                int node = nodeBll.SelFirstNodeID(NodeID);//获取最终父节点
                M_Node Pnid = nodeBll.SelReturnModel(node);
                string NodeDir = "";
                if (nodeinfo.HtmlPosition > 0 && nodeinfo.ParentID > 0)
                {
                    if (nodeinfo.NodeType == 1)
                ...
                if (thisnodeid > 0)
                {
                    if (maxpagenum > 0)
                    {
                        for (int p = 1; p <= maxpagenum; p++)
                        {
                            ...
                            if (p == 1) { filename = ""; }
                            string nodepagenuls = "";
                            if (!string.IsNullOrEmpty(nodeBll.SelReturnModel(thisnodeid).NodeUrl))
                            {
                                if (nodeBll.SelReturnModel(thisnodeid).NodeUrl.IndexOf(".") > -1)
                                {
                                    string[] nodearr = nodeBll.SelReturnModel(thisnodeid).NodeUrl.Split(new string[] { "." }, StringSplitOptions.None);

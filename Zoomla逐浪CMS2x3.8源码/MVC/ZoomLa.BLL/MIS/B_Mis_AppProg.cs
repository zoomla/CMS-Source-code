...
namespace ZoomLa.BLL
{
    public class B_Mis_AppProg
    {
        ...
        public PageSetting SelPage(int cpage, int psize, int appid = -100, int stepnum = -100)
        {
            string where = " 1=1";
            if (appid != -100) { where += " AND　AppID=" + appid; }
            if (stepnum != -100) { where += " AND ProLevel=" + stepnum; }
            PageSetting setting = PageSetting.Single(cpage, psize, TbName, "A." + PK, where, "");
            setting.fields = "A.*,B.HoneyName,B.UserName";
            setting.t2 = "ZL_User";
            setting.on = "A.ApproveID=B.UserID";

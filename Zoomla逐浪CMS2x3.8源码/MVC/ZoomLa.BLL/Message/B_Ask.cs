...
namespace ZoomLa.BLL
{
    public class B_Ask
    {
        ...
        public PageSetting SelPage(int cpage, int psize, string addon)
        {
            string where = "1=1";
            switch (addon)
            {
                case "0": where += " AND Elite=1"; break;
                case "2": where += " AND Score<>0"; break;
            }
            PageSetting setting = PageSetting.Single(cpage, psize, strTableName, PK, where, "ID desc");
            DBCenter.SelPage(setting);
            return setting;
        }
        ...
        public PageSetting SelByIDS(int cpage, int psize, string qids)
        {
            if (!string.IsNullOrEmpty(qids)) { SafeSC.CheckIDSEx(qids); }
            string where = "ID IN (" + qids + ")";
            PageSetting setting = PageSetting.Single(cpage, psize, strTableName, PK, where, "AddTime DESC");
            DBCenter.SelPage(setting);
            return setting;
        }

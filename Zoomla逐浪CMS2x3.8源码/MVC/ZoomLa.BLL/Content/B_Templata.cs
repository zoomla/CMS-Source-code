...
namespace ZoomLa.BLL
{
    ...
    public class B_Templata
    {
        ...
        {
            return Sql.UpdateByID(strTableName, model.PK, model.TemplateID, BLLCommon.GetFieldAndPara(model), model.GetParameters());
        }
        ...
        public PageSetting ReadUserall_Page(int cpage, int psize, int ParentID, int UserID, int UserGroup)
        {
            string where = "(UserGroup=@UserGroup and ParentID=@ParentID and userid=0)  or (userid=@Userid and ParentID=@ParentID and UserGroup=@UserGroup)";
            List<SqlParameter> sp = new List<SqlParameter>();
            sp.Add(new SqlParameter("ParentID", ParentID));
            sp.Add(new SqlParameter("UserID", UserID));
            sp.Add(new SqlParameter("UserGroup", UserGroup));

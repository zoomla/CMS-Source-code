namespace ZoomLa.BLL
{
    ...
    public class B_Pub
    {
        ...
        public PageSetting GetComments_SPage(int cpage, int psize, string TBName, string inputer)
        {
            string where = "PubInputer=@inputer and Parentid=0";
            List<SqlParameter> sp = new List<SqlParameter>() { new SqlParameter("inputer", inputer) };
            PageSetting setting = PageSetting.Single(cpage, psize, TBName, "ID", where, "", sp);
        ...
        public PageSetting SelPage(int cpage, int psize, string addon = "")
        {
            ...
            if (!string.IsNullOrEmpty(addon))
            {
                ...
                {
                    case "notdel":
                        where += " AND PubIsDel=0 and (PubSiteID is null or PubSiteID='')";
                        break;
                }
            }

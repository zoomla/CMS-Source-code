...
namespace ZoomLa.SQLDAL.SQL
{
    public class PageSetting
    {
        ...
        public int pageCount { get { return _pageCount < 1 ? 1 : _pageCount; } set { _pageCount = value; } }
        ...
        /// AJAX提交的参数筛选器
        /// </summary>
        public string mvcparam=".mvcparam";
        //--------------------分页辅助2
        public string page_mode = "";//页面模式,根据不同的模式,进行微小的修正
        public string page_wrap_start = "";//是否指定外层包装
        public string page_wrap_end = "";
        //--------------------辅助方法
        /// <summary>
        /// 主用于标签解析,可自定义别名
        /// </summary>
        ...
        public static PageSetting Single(int cpage, int psize, string tbname, string pk, string where, string order = "", List<SqlParameter> sp = null, string fields = "*")
        {
        ...
        public static PageSetting Double(int cpage, int psize, string t1, string t2, string pk, string on, string where, string order = "", List<SqlParameter> sp = null, string fields = "*")
        {
            if (string.IsNullOrEmpty(order)) { order = pk + " DESC"; }
            return new PageSetting()
            {

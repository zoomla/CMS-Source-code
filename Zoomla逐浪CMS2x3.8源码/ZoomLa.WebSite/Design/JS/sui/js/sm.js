 ...
 * =====================================================
 */
;Zepto.smVersion = "0.6.2";+function ($) {
    "use strict";
...
+function ($) {
    ...
    Zepto.toast = function(msg, duration, extraclass) {
        ...
        Zepto.openModal($toast, function(){
            setTimeout(function() {
                Zepto.closeModal($toast);
            }, duration || 2000);
        });
    };
    ...
    var Calendar = function (params) {
        ...
        p.initCalendarEvents = function () {
            ...
            if (p.params.touchMove) {
                 ...
                 * Start - edit by JSoon
                 */
                p.yearsPickerWrapper.on(Zepto.touchEvents.start, handleYearTouchStart);
                p.yearsPickerWrapper.on(Zepto.touchEvents.move, handleYearTouchMove);
                p.yearsPickerWrapper.on(Zepto.touchEvents.end, handleYearTouchEnd);
                /**
                 * Start - edit by JSoon
                 */
        ...
        p.open = function () {
            ...
            if (!p.opened) {
                ...
                if (p.inline) {
                    ...
                    p.container = $(Zepto.pickerModal(p.pickerHTML));
                    $(p.container)
                        .on('close', function () {
                            onPickerClose();
                        });
...
+ function($) {
    ...
    function Plugin(option) {
        ...
        this.each(function() {
            ...
            var options = Zepto.extend({}, $this.dataset(), typeof option === 'object' && option);
...
+function ($) {
    ...
    Zepto.openPanel = function (panel) {
        ...
        // Transition End;
        var transitionEndTarget = effect === 'reveal' ? $(Zepto.getCurrentPage()) : panel;
    ...
    Zepto.initSwipePanels = function () {
        ...
        $(document).on(Zepto.touchEvents.start, handleTouchStart);
        $(document).on(Zepto.touchEvents.move, handleTouchMove);
        $(document).on(Zepto.touchEvents.end, handleTouchEnd);
    };
...
+function($) {
    ...
    Zepto.initPage = function(page) {
        ...
        Zepto.initPullToRefresh($content);
        Zepto.initInfiniteScroll($content);
        Zepto.initCalendar($content);

        //extend
        if (Zepto.initSwiper) Zepto.initSwiper($content);
    };
...
+ function($) {
    ...
    if(Zepto.device.ios){
        ...
        $(document).on(Zepto.touchEvents.move, ".page-current .bar",function(){
            event.preventDefault();
        });
        //监听ajax页面跳转

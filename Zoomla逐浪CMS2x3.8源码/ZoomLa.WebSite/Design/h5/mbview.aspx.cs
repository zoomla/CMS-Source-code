...
public partial class Design_h5_mbview : System.Web.UI.Page
{
    ...
    private M_Design_Page GetScenceMod()
    {
        ...
        if (Sid > 0) { seMod = pageBll.SelReturnModel(Sid); }
        else if (TlpID > 0)
        {
            seMod = pageBll.SelModelByTlp(TlpID);
        }
        else if (!string.IsNullOrEmpty(Mid))
        {
            seMod = pageBll.SelModelByGuid(Mid);
        }
        //场景不存在或处理回收站状态
        if (seMod == null || seMod.ID < 1 || seMod.Status == (int)ZLEnum.ConStatus.Recycle)
        {
            ...
            seMod = pageBll.SelModelByTlp(tlpMod.ID);

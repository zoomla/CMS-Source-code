...
public partial class Plat_Admin_UserAudit : System.Web.UI.Page
{
            ...
            case "agree":
                {
                    ...
                    //检测会员是否存在,如不存在,则首先升级为能力中心用户
                    if (!DBCenter.IsExist(upMod.TbName, "UserID=" + ualyMod.UserID))
                    {
                        M_UserInfo mu = buser.SelReturnModel(ualyMod.UserID);
                        M_User_Plat newmu = upBll.NewUser(mu, compMod);
                        upBll.Insert(newmu);
                    }
                    else //已存在的会员,则直接更新公司与角色信息
                    {
    ...
    protected void BatAgree_Btn_Click(object sender, EventArgs e)
    {
        ...
        {
            //检测会员是否存在,如不存在,则首先升级为能力中心用户
            if (!DBCenter.IsExist(upMod.TbName, "UserID=" + dr["UserID"]))
            {
                M_UserInfo mu = buser.SelReturnModel(DataConverter.CLng(dr["UserID"]));
                M_User_Plat newmu = upBll.NewUser(mu, compMod);
                upBll.Insert(newmu);
            }
            else //已存在的会员,则直接更新公司与角色信息

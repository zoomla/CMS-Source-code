...
public partial class manage_Shop_OrderStateManage : CustomerPageAction
{
    ...
    private void ReadXml(int type)
    {
        ...
        doc.Load(Server.MapPath("../../Config/DelityConfige.xml"));
        XmlNode xmlnode = doc.SelectSingleNode("deconfi");
        if (type == 1 || type == 2)  //全部付款/部分付款
        {
            txtContent.Text = xmlnode.SelectSingleNode("orderConfi/paystate1").InnerText;
        }
        else if (type == 4)  //退款
        {
            txtContent.Text = xmlnode.SelectSingleNode("orderConfi/paystate2").InnerText;
        }
        else
        {
            txtContent.Text = "";

...
public partial class Install_Default : System.Web.UI.Page
{
    ...
    public void CheckEnviorment()
    {
        ...
        string[] dlls = "ZoomLa.Ajax,ZoomLa.BLL,ZoomLa.Controls,ZoomLa.Components,ZoomLa.Common,ZoomLa.Web,ZoomLa.SQLDAL,ZoomLa.Model,MySql.Data".Split(',');
        string[] files = new string[] { "AppSettings.config", "ConnectionStrings.config", "AuditData.xml", "Cartinfo.xml", "CRM_Dictionary.xml", "MakeSendList.config", "ModuleConfig.config", "log.config", "Pages.config" };
        string[] files2 = new string[] { "URLRoute.config", "globalization.config", "Guest.config", "URLRewrite.config", "Site.config", "SystemRoot.config", "StationGroup.config", "WaterConfig.config" };
        //检测环境
    ...
    {
        string xmlPath = Server.MapPath("/Config/ConnectionStrings.config");
    ...
    //为xml元素添加属性
    private void AddXmlAttr(XmlDocument xmldoc, XmlNode node, string name, string vlaue)
    {
        name = name.Trim();

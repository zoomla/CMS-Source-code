...
public class Call
{
    ...
    public static MvcHtmlString MVCLabel(string ntext) 
    {
    ...
    /// <param name="flag">true:检测空值,false:不检测</param>
    /// <param name="fields">如不为空,则非其中的字段不允许修改</param>
    public DataTable GetDTFromMVC(DataTable dt, HttpRequestBase req, bool flag = true)
    {
            ...
            try
            {
                ...
                else
                {
                    if (DataConverter.CBool(dr["IsNotNull"].ToString()) && flag)
                    {
                        ...
                        {
                            throw new Exception(dr["FieldAlias"].ToString() + "不能为空!");
                        }
                    }
                    ...
                    switch (dr["FieldType"].ToString())
                    {
                        case "FileType"://其与MultiPicType一样，都是读页面上的隐藏控件,而非<select>中的值,存在一个隐藏字段中
                            ...
                            if (chksize && sizefield != "")
                            {
                                ...
                                row1[0] = sizefield;
                                row1[1] = "FileSize";
                                row1[2] = req.Form["txt_" + sizefield];
                            ...
                            rowr1[1] = "FileType";
                            rowr1[2] = req.Form["txt_" + dr["FieldName"]];
                        ...
                        case "MultiPicType":
                            ...
                            }
                            {
                                ...
                                rowr2[2] = req.Form["txt_" + dr["FieldName"]];
                        ...
                        case "NumType":
                            #region
                            double value = DataConverter.CDouble(string.IsNullOrEmpty(req.Form["txt_" + dr["FieldName"]]) ? "0" : req.Form["txt_" + dr["FieldName"]]);//避免空字符串报错
                        ...
                        case "Images"://组图
                            fdr[0] = dr["FieldName"].ToString();
                            fdr[1] = "Images";
                            fdr[2] = (req.Form["txt_" + dr["FieldName"].ToString()] ?? "").Trim(',');
                            table.Rows.Add(fdr);
                            break;
                        case "CameraType":
                            fdr[0] = dr["FieldName"].ToString();
                        ...
                        case "autothumb":
                            {
                                ...
                                string topimg = req.Form["ctl00$Content$ThumImg_Hid"];
                        ...
                        default:
                            ...
                            switch (ftype)
                            {
                                ...
                                case "MapType":
                                    ...
                                    break;
                                case "Charts":
                                    if (req.Form[dr["FieldName"].ToString()] == null || req.Form[dr["FieldName"].ToString()] == "0")
                                    {
                                        fvalue = "0";
                                    }
                                    else
                                    {
                                        fvalue = req.Form[dr["FieldName"].ToString()];
                                    }
                                    break;
                                case "SqlType":
                                    if (req.Form["txt_" + dr["FieldName"].ToString()] == null || req.Form["txt_" + dr["FieldName"].ToString()] == "")
                                    {
                                        fvalue = "";
                                    }
                                    else
                            ...
                            string serPath = req.PhysicalApplicationPath;
                            ...
                            //        fvalue = req.Form["txt_content"];
                            //        foreach (string str in remoteImageList)
                            //        {
                            //            string imgurl = req.PhysicalApplicationPath + WaterModuleConfig.WaterConfig.imgLogo;
                            //            //新图片名称
                            //            string newImgname;
                            //            string Patha;
                            ...
                            //                try
                            //                {
                            //                    imgInfo = mfieldBll.downRemoteImg(function.VToP(Patha), str, newImgname, Patha, imgurl);

...
namespace ZoomLa.BLL
{
    public class B_GuestBookCate : ZL_Bll_InterFace<M_GuestBookCate>
    {
        ...
        /// <returns></returns>
        public PageSetting SelPage(int cpage, int psize, int status = -100, int cateid = -100, int pid = -100, string skey = "", int userid = 0, bool isShowListByAudit = false)
        {
            string where = " 1=1";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (status != -100) { where += " AND A.Status=" + status; }
            else { where += " AND A.Status<>-1"; }
            if (cateid > 0 & status != -1) { where += " AND A.CateID=" + cateid; }
            ...
            //根据留言分类显示模式配置筛选，用于留言中心显示留言
            if (isShowListByAudit)
            {
                where += " AND (A.Status!=0 OR B.IsShowUnaudit=1";
                if (userid > 0) { where += " OR A.Userid=" + userid; }
                where += ")";
            }
            PageSetting setting = PageSetting.Double(cpage,psize,"ZL_GuestBook","ZL_GuestCate","A.GID","A.CateID=B.CateID",where,"GDate DESC",sp);
            setting.fields = "A.*,B.CateName,(SELECT UserName FROM ZL_User WHERE ZL_User.UserID=A.UserID) UserName,(SELECT HoneyName FROM ZL_User WHERE ZL_User.UserID=A.UserID) HoneyName,(SELECT salt FROM ZL_User WHERE ZL_User.UserID=A.UserID) UserFace";

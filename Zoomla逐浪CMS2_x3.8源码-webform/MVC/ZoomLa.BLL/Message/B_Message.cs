namespace ZoomLa.BLL
{
    ...
    public class B_Message
    {
        ...
        public PageSetting SelPage(int cpage, int psize, int msgtype = -100, string title = "")
        {
            string where = " 1=1";
            List<SqlParameter> sp = new List<SqlParameter>();
            if (msgtype != -100) { where += " AND MsgType=" + msgtype; }
            if (!string.IsNullOrEmpty(title)) { where += " AND Title LIKE @title"; sp.Add(new SqlParameter("title", title)); }
            PageSetting setting = PageSetting.Single(cpage, psize, strTableName, PK, where, "PostDate DESC", sp);

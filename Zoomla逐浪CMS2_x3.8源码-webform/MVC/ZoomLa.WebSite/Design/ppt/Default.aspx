<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="ZoomLaCMS.Design.ppt.Default" MasterPageFile="~/Common/Master/Empty2.master" %>
<asp:Content runat="server" ContentPlaceHolderID="head">
<link href="/design/res/css/main.css" rel="stylesheet" />
<link href="css/design.css" rel="stylesheet" />
<title>工作台--PPT在线设计</title>
</asp:Content>
...
<div id="slist_div" runat="server" visible="false">
        ...
        <div class="conlist">
            <ul class="list-unstyled">
                <ZL:ExRepeater ID="RPT" runat="server" PageSize="9"  PagePre="<tr><td colspan='12'><div class='text-center'>" PageEnd="</div></td></tr>">
                    <ItemTemplate>
                        <li title="点击前往设计" onclick="location='/design/ppt/default.aspx?id=<%#Eval("Guid") %>'">
                            <div class="imgdiv">
    ...
    <script>
        $(function(){
            //插入新建场景按钮
            var addtlp ="<li title='点击新建PPT' onclick='ShowComDiag(\"/design/ppt/diag/add.aspx?device=pc\", \"新建PPT\");'><div class='imgdiv'><i class='fa fa-plus-square-o'></i></div><span><strong>创建新场景</strong></span></li>";
            var $li= $(".conlist ul li:last");
            if($li.length<1){$(".conlist ul").append(addtlp)}
            else{  $li.after(addtlp)}
        })
    </script>
</div>
<div id="edit_div">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                ...
                <li class="cmd_addpage">
                    ...
                    <ul class="create_ul">
                        <li><a href="javascript:;" onclick="ShowComDiag('/design/diag/add.aspx','新建PPT');">新建PPT</a></li>
                        <li><a href="javascript:;" onclick='ShowComDiag("/Design/mbh5/AddScence.aspx?device=pc", "新建场景");'>自由场景</a></li>
                        <li><a href="javascript:;" onclick="ShowComDiag('/Design/mbh5/FastCreate.aspx', '智能相册');">智能相册</a></li>
                        <li><a href="/Design/Ask/Default.aspx#/tab/ask_add">微调查</a></li>
                        <li><a href="/Design/Mobile/Welcome.aspx">微站</a></li>
                    </ul>
                </li>
            ...
            <ul class="nav navbar-nav navbar-right">
                ...
                <li><a href="/design/ppt/pcview.aspx?sid=<%:pageMod.ID %>" target="_blank"><i class="fa fa-laptop"></i> PC预览</a></li>
...
<div class="mbdesign" style="background-color:#fff;">
    <div class="rpane left">
        <div class="tab_head text-center">
            <button type="button" class="btn btn-sm btn-info" title="页面配置" onclick='ShowComDiag("diag/add.aspx?id=<%:pageMod.guid %>", "新建场景");'><i class="fa fa-file"></i></button>
    ...
    <div class="mainpane">
        ...
        <div class="sidebar" style="left:-39px;">
            ...
            <a href="javascript:;" class="btn btn-sm btn-default cmd_conf"><i class="fa fa-cog"></i></a>
        </div>
    </div>
</div>
<div id="diagBody">
...
$(function () {
    ...
    $(".cmd_page:first").click(function(){CloseDiag();ShowDiag_L("/design/diag/se_page.aspx?id=<%:Mid%>", "PPT配置");});
    $(".cmd_conf:first").click(function(){CloseDiag();ShowDiag_L("/design/diag/se_conf.aspx?id=<%:Mid%>", "PPT设置");});
    ...
    seajs.use(["/design/js/se_comp/page"], function (instance) {
        page = instance;
        page.apiurl="/design/ppt/ppt.ashx";
    ...
    $("#preview_btn").popover({animation: true,html:true,trigger:"hover",placement: 'left',
        content:function(){return '<div style="width:170px;padding:10px;"><img src="/common/common.ashx?url=<%=ZoomLa.Components.SiteConfig.SiteInfo.SiteUrl+"/design/ppt/pcview.aspx?sid="+pageMod.ID%>" style="width:150px;height:150px;" /><div class="text-center" style="margin-top:5px;">请先保存再扫描预览</div></div>';}});
    ...
    //----------------------------
    var scence={
        list:[],//数据,order与id均从1开始
        conf:{},//存储总配置,分配置另存(图标,切换方式)
        $body:$("#page_ul"),
...
//bgs与li的data-id关联
scence.ppt={bgs:{}};
scence.ppt.loadAllbgFromCache=function(){
    var ref=this;
    $("#page_ul li").each(function(){
        ref.setbg($(this).data("id"));
    });
}
scence.ppt.loadbg=function(dom,index){
    //更新数组和li
    html2canvas(dom, {onrendered: function (canvas) {
        var image = canvas.toDataURL("image/png");
        var id=scence.list.GetByID(index,"swipe_index").id;
        scence.ppt.bgs[id]=image;
        scence.ppt.setbg(id);
    }});
}
scence.ppt.setbg=function(id)
{
    $("#page_ul li[data-id='"+id+"']").css("background-image","url("+scence.ppt.bgs[id]+")");
}
//启动时初始化背景(动画元素无法正常绘入)
scence.ppt.initbg=function(){
    var $selis=$("#page_ul li");
    var loadbg=function($lis,index,callback){
        var $li=$($lis[index]);
        var id=$li.data("id");
        if($li.length<1){callback();console.log("end",index); return;}
        IfrHelper.find(".swiper-slide").hide();
        //---------------------------------------------------------
        $dom=IfrHelper.find("#section_"+id);
        $dom.show();
        html2canvas($dom[0], {onrendered: function (canvas) {
            var image = canvas.toDataURL("image/png");
            scence.ppt.bgs[id]=image;
            loadbg($lis,(index+1),callback);}
        });
    }
    loadbg($selis,0,function(){
        IfrHelper.find(".swiper-slide").show();
        for (var i = 0; i < $selis.length; i++) {
            scence.ppt.setbg($($selis[i]).data("id"));
        }
    });
}
//----------------------------提供资源的筛选功能

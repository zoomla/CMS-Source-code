        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Mid > 0)
                {
                    ...
                    conMod.pic = "";
                    conMod.ParticIDS = model.Partic;
                    conMod.ParticNames = buser.GetUserNameByIDS(model.Partic);
                    conMod.EDate = model.EDate.ToString("yyyy/MM/dd HH:mm");
                    Save_Hid.Value = JsonConvert.SerializeObject(conMod);
                    CUser_Hid.Value = model.CUName;
                }
                else
                {
                    CUser_Hid.Value = buser.GetLogin().UserName;
                }
                //else
                //{
                //    if (NodeID < 1) { function.WriteErrMsg("未指定项目"); }
                //}
            }
        }
        private void MyBind() { }

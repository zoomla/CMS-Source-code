...
{
    public partial class FlowAudit : System.Web.UI.Page
    {
        ...
        bool disall = false;//发起人查看时,或抄送人查看
        ...
        public M_MisProLevel GetNextLevel()
        {
            ...
            else//固定流程(后台定义)
            {
                ...
                if (oaMod.CurStepNum == 0)//步骤为0时，加载第一步,默认为0    ,progDT.Rows.Count < 1||为空或(除去此判断,我们维护好CurrentStepNum)
                {
                    ...
                    {
                        function.WriteErrMsg("流程下未指定步骤,请联系管理员!!!");
                    }
        ...
        public void ShowPage(M_OA_Document oaMod)
        {
            ...
            if (disall) showBll.config.EnableField = "";
